...
@interface FLTAssetPickResult ()
+ (FLTAssetPickResult *)fromList:(NSArray *)list;
+ (nullable FLTAssetPickResult *)nullableFromList:(NSArray *)list;
- (NSArray *)toList;
@end
...
@implementation FLTAssetPickResult
+ (instancetype)makeWithPath:(NSString *)path
    localIdentifier:(nullable NSString *)localIdentifier {
  FLTAssetPickResult* pigeonResult = [[FLTAssetPickResult alloc] init];
  pigeonResult.path = path;
  pigeonResult.localIdentifier = localIdentifier;
  return pigeonResult;
}
+ (FLTAssetPickResult *)fromList:(NSArray *)list {
  FLTAssetPickResult *pigeonResult = [[FLTAssetPickResult alloc] init];
  pigeonResult.path = GetNullableObjectAtIndex(list, 0);
  pigeonResult.localIdentifier = GetNullableObjectAtIndex(list, 1);
  return pigeonResult;
}
+ (nullable FLTAssetPickResult *)nullableFromList:(NSArray *)list {
  return (list) ? [FLTAssetPickResult fromList:list] : nil;
}
- (NSArray *)toList {
  return @[
    self.path ?: [NSNull null],
    self.localIdentifier ?: [NSNull null],
  ];
}
@end
...
- (nullable id)readValueOfType:(UInt8)type {
  switch (type) {
    case 128: 
      return [FLTAssetPickResult fromList:[self readValue]];
    case 129: 
      return [FLTAssetPickResult fromList:[self readValue]];
    case 130: 
...
- (void)writeValue:(id)value {
  if ([value isKindOfClass:[FLTAssetPickResult class]]) {
    [self writeByte:128];
    [self writeValue:[value toList]];
  } else if ([value isKindOfClass:[FLTAssetPickResult class]]) {
    [self writeByte:129];
    [self writeValue:[value toList]];
...
void SetUpFLTImagePickerApi(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FLTImagePickerApi> *api) {
  {
    ...
    if (api) {
      ...
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        ...
        [api pickImageWithSource:arg_source maxSize:arg_maxSize quality:arg_imageQuality fullMetadata:arg_requestFullMetadata completion:^(FLTAssetPickResult *_Nullable output, FlutterError *_Nullable error) {
  ...
  }
  {
    ...
    if (api) {
      ...
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        ...
        [api pickMultiImageWithMaxSize:arg_maxSize quality:arg_imageQuality fullMetadata:arg_requestFullMetadata limit:arg_limit completion:^(NSArray<FLTAssetPickResult *> *_Nullable output, FlutterError *_Nullable error) {
          callback(wrapResult(output, error));
        }];
      }];
    } else {
  ...
  /// Selects images and videos and returns their paths and local identifiers.
  {
    ...
    if (api) {
      ...
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        ...
        [api pickMediaWithMediaSelectionOptions:arg_mediaSelectionOptions completion:^(NSArray<FLTAssetPickResult *> *_Nullable output, FlutterError *_Nullable error) {
          callback(wrapResult(output, error));
        }];
      }];
    } else {
